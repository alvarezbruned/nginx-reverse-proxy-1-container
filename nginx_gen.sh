#!/bin/bash
var=`cat ./docker-compose.yml | grep website_ | cut -d '_' -f 2 | cut -d ':' -f 1`
DOMAINS=( $var )
TOTAL=${#DOMAINS[@]}
I=$TOTAL
...
do
  ...
  echo $DOMAIN
  ./bind.sh $DOMAIN
  CONTAINER="$(docker ps | grep websites | cut -d ' ' -f 1)"
  ...
  echo '    root   /var/www/html/site'$ITERATION';' >> $OUTFILE3
